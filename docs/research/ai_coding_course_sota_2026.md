...
- **Anthropic now documents a spec step directly** — the "Let Claude interview you" section of the Claude Code best-practices page supplies a verbatim interview prompt (`Interview me in detail using the AskUserQuestion tool… then write a complete spec to SPEC.md`), four criteria for a good spec (self-contained · names the files and interfaces · states what is out of scope · ends with an end-to-end verification step), and the instruction to "start a fresh session to execute it." It sits *outside* the Explore→Plan→Implement→Commit workflow, which the same page bounds with "if you could describe the diff in one sentence, skip the plan." This is the vendor-native anchor the rest of this section was missing. [primary] https://code.claude.com/docs/en/best-practices
- **GitHub Spec Kit** — most-adopted open-source SDD toolkit (MIT, Python CLI): Constitution → Specify → Plan → Tasks → Implement, all as committed markdown, supporting 30+ agents. Commands are **namespaced** — `/speckit.constitution`, `/speckit.specify`, `/speckit.plan`, `/speckit.tasks`, `/speckit.implement`. 124.3k stars as of 2026-07-28. *(A previously cited "v0.13.0, July 2026" version string could not be verified — do not repeat it.)* [primary] https://github.com/github/spec-kit
...
- **Birgitta Böckeler** (on martinfowler.com, **2025-10-15**) gives the best conceptual frame: three maturity levels — **spec-first** → **spec-anchored** → **spec-as-source**. Her warning about the third is sharper than the usual paraphrase: an LLM-based spec-as-source risks combining "the downsides of both MDD and LLMs: Inflexibility *and* non-determinism." This is the authoritative primary analysis to assign. [primary] https://martinfowler.com/articles/exploring-gen-ai/sdd-3-tools.html
- **Harness evidence** — Anthropic's planner/generator/evaluator experiment shows why separation works ("separating the agent doing the work from the agent judging it proves to be a strong lever") *and* supplies the caveat that specs should stay at "product context and high level technical design rather than detailed technical implementation," because premature detail propagates errors downstream. [primary] https://www.anthropic.com/engineering/harness-design-long-running-apps

→ **Full treatment, with the two-altitude teaching model and a week-by-week placement table: [`spec-driven-development.md`](spec-driven-development.md).**
...
- ✅ **[IMPLEMENTED 2026-07-28 — W10 + workshop S3]** **Add spec-driven development** as a first-class practice. Landed as a *two-altitude* model (feature-level spec loop wrapping task-level EPIC) rather than a fifth phase, taught via Anthropic's interview prompt rather than a Spec Kit install; Spec Kit and Böckeler's maturity model are taught as the productized form and the conceptual frame in W10. See [`spec-driven-development.md`](spec-driven-development.md). [primary] https://code.claude.com/docs/en/best-practices · https://github.com/github/spec-kit · https://martinfowler.com/articles/exploring-gen-ai/sdd-3-tools.html
